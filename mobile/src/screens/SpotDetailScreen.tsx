...
export default function SpotDetailScreen({ route, navigation }: any) {
  ...
  const [budget, setBudget] = useState(String((user?.preferences as any)?.budget || '20000'));
  ...
  const load = async () => {
    ...
    try {
      if (s?.latitude && s?.longitude) {
        setEnv(await api(`/api/spots/environment?latitude=${s.latitude}&longitude=${s.longitude}`, {}, false));
      }
    } catch {}
  ...
  const budgetNum = Number(budget) || 20000;
  const stayNights = 2;
  const stayTotalFor = (h: any) => Number(h.totalForStay || Number(h.pricePerNight) * stayNights);
  const recommended = hotels.filter((h) => stayTotalFor(h) <= budgetNum);
  const others = hotels.filter((h) => stayTotalFor(h) > budgetNum);
  const orderedHotels = [...recommended, ...others];
  ...
  const bookHotel = async (hotel: any) => {
    try {
      ...
      const held: any = await api('/api/hotels/book', {
        method: 'POST',
        body: JSON.stringify({ hotelId: hotel.id, startDate: '2026-10-12', endDate: '2026-10-15', budget: budgetNum }),
      });
      ...
      if (e?.status === 409 || /already.*confirmed|clash|overlap/i.test(e.message || '')) {
        Alert.alert('Date clash', 'A plan in that particular duration is already confirmed. Please choose different dates.');
      } else {
        Alert.alert('Booking failed', e.message);
      }
    } finally { setBusyHotel(null); }
  };
  ...
  return (
    ...
    <ScrollView style={s.wrap} contentContainerStyle={{ paddingBottom: 40 }}>
      ...
      <View style={s.card}>
        <Text style={s.cardH}>Weather</Text>
        <Text style={s.line}>{env ? `${env.weather || '—'} · ${env.temperature ?? '—'}°C (source: ${env.source || 'live'})` : 'Loading live weather…'}</Text>
        <Text style={s.cardH}>Air Quality (AQI)</Text>
        <Text style={s.line}>{env ? `AQI ${env.aqi} · ${env.aqiLabel || ''}` : 'Loading live AQI…'}</Text>
        <Text style={s.cardH}>Crowd Status</Text>
        {crowd ? (
          <>
            <Text style={s.line}>Crowd forecast · {crowd.level} ({crowd.crowdScore})</Text>
            {(crowd.explanation || []).map((e: string, i: number) => <Text key={i} style={s.line}>• {e}</Text>)}
            <Text style={s.model}>Model: {crowd.model}</Text>
          </>
        ) : (
          <Text style={s.line}>Loading crowd forecast…</Text>
        )}
      </View>
      {price && (
        <View style={s.card}>
      ...
      <View style={s.card}>
        <Text style={s.cardH}>Choose your budget</Text>
        <Text style={s.line}>Hotels with a {stayNights}-night stay within your budget are recommended first.</Text>
        <TextInput style={s.input} placeholder="Budget ₹" keyboardType="numeric" value={budget} onChangeText={setBudget} />
        <Text style={s.recoCount}>{recommended.length} of {hotels.length} fit ₹{budgetNum.toLocaleString('en-IN')}</Text>
      </View>

      <View style={s.card}>
        <Text style={s.cardH}>Hotels recommended for your budget ({hotels.length})</Text>
        {hotels.length === 0 && <Text style={s.line}>No hotels yet — run the seed (npm run prisma:seed).</Text>}
        {orderedHotels.map((h: any) => {
          const fits = stayTotalFor(h) <= budgetNum;
          return (
          <View key={h.id} style={s.hotel}>
            ...
            <Text style={[s.recoBadge, fits ? s.recoYes : s.recoNo]}>{fits ? `✓ Recommended for ₹${budgetNum.toLocaleString('en-IN')}` : `Over budget (${stayNights}n ≈ ₹${stayTotalFor(h).toLocaleString('en-IN')})`}</Text>
            ...
            <View style={s.hotelRow}>
              <View>
                ...
                <Text style={s.stayTotal}>{stayNights}n stay ≈ ₹{stayTotalFor(h).toLocaleString('en-IN')}</Text>
              </View>
      ...
      <Pressable style={s.plan} onPress={() => navigation.navigate('Planner', { spotId: id, spotName: spot.name, budget: budgetNum })}>
        <Text style={s.planT}>Plan the trip →</Text>
      </Pressable>
      </View>
    </ScrollView>
    </Screen>
...
const s = StyleSheet.create({
  ...
  recoCount: { fontSize: 12, fontWeight: '800', color: theme.ok, marginTop: 6 },
  recoBadge: { fontSize: 11, fontWeight: '800', marginTop: 6 },
  recoYes: { color: theme.ok },
  recoNo: { color: theme.muted },
